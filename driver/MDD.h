...
class MDD
{
    ...
    */
        MDD(PinName A1,PinName B1,PinName A2,PinName B2,PinName A3,PinName B3,int pls,double p,double i,double d,double max)
        {
            this->p = p;
            this->i = i;
            this->d = d;
            ...
            for(int i = 0;i<3;i++)PIDs[i] = new PIDController(this->p,this->i,this->d);
            for(int i = 0;i<3;i++)PIDs[i]->setOutputLimit(max);
        };
        MDD(QEI *qeis,double p,double i,double d,double max)
        {
            ...
            this->i = i;
            this->d = d;
            for(int i = 0;i<3;i++)QEIs[i] = &qeis[i];
            for(int i = 0;i<3;i++)PIDs[i] = new PIDController(this->p,this->i,this->d);
            for(int i = 0;i<3;i++)PIDs[i]->setOutputLimit(max);
        };
